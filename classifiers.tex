...
%Haltead metrics are metric based on the number of operands and operators.
%For example, the sum of operands and operators is the program length. If we sum only distincs, we get the program vocabulary.
\begin{frame}
...
%The statistics we will compute for experiments is:
%the probability of detection, pourcentage of correctly classified ARB-prone instances,
%the probability of false alarm, pourcentage of uncorrectly classified non-ARB-prone instances,
%the balance, a compromise between them because a good probability detection doesn't mean a good classifier if it includes too much false alarms.
...
%For MySQL, we can also say that Naive Bayes with logarithmic scale give us the highest probability of detection, false alarm ans balance.
\begin{frame}
 \frametitle{Results without new metrics}
 \framesubtitle{MySQL}
 \begin{center}
